...
form.addEventListener("submit", function (e) {
  ...
  e.preventDefault(); //this prevents the submission from submitting to a server

  const value = parseInt(amount.value); //parseint converts the input value from the form to string
  const random = Math.floor(Math.random() * text.length);


  //the code below is if the input is not a number or teh number is  larger than 9 and less than 0.
  //display a single paragraph or the next code
